...
def main():
    ...
    if args.do_train:
        ...
        train_output_file = os.path.join(args.output_dir, "train_results.txt")
        epoch_losses = []
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                with open(train_output_file, "w") as writer:
                    logger.info("Epoch: %s, Step: %s, Train Loss: %s", str(epoch), str(nb_tr_steps), str(tr_loss))
            ...
            epoch_losses.append(str(tr_loss))   

            with open(train_output_file, "w") as writer:
                    logger.info("\n")
                    logger.info("Epoch losses so far")
                    for enumm, train_loss_str in enumerate(epoch_losses):
                        logger.info("Epoch: %s, Train Loss: %s", str(enumm), str(train_loss_str))
                    logger.info("\n")
